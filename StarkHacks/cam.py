...
def open_camera(index: int) -> Optional[cv2.VideoCapture]:
    ...
    cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)  # DSHOW opens in <1s vs MSMF's 10-30s
    if not cap.isOpened():
        return None
...
def main() -> None:
    ...
    cap = open_camera(1)  # Brio 105 is typically index 1
    if cap is None:
        print("Index 1 failed. Scanning all cameras...")
