...
For live capture, you can also pass `--source` directly:
...
# Linux device path
python main.py --source /dev/v4l/by-id/<camera-id>

# Auto-scan camera indices
python main.py --source auto
```

### Optional: Dual-Camera Parallax Distance (macOS-friendly)

This repository now includes an optional stereo script:

```bash
python stereo_parallax.py --source-left 0 --source-right 1 --baseline-m 0.20
```

This mode:
- Runs YOLO person detection on both cameras
- Matches people across cameras using geometry + appearance
- Assigns one fused global ID (`GID`) to the matched left/right pair
- Computes disparity and depth (`Z`)
- Reports distance from each camera sensor (`L` and `R`)

You can provide a calibrated focal length directly:

```bash
python stereo_parallax.py --source-left 0 --source-right 1 --baseline-m 0.20 --focal-px 1400
```
